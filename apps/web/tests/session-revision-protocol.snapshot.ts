...
describe('session revision Host Remote protocol', () => {
  ...
  it('snapshots stale, committed, and withdrawn-target calls through the shipped Web Host', async () => {
    ...
    const invoke = async (rpcId: string, request: unknown): Promise<void> => {
      ...
      const endpoint = 'sessionRevision/withdraw'
    ...
    await invoke('revision-stale', { targetSeq: 3, expectedLastSeq: 4, timezone: 'Asia/Shanghai' })
    await invoke('revision-commit', { targetSeq: 3, expectedLastSeq: 9, timezone: 'Asia/Shanghai' })
    await invoke('revision-withdrawn', { targetSeq: 3, expectedLastSeq: 10, timezone: 'Asia/Shanghai' })
