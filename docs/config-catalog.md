...
- `@deepseek-ai/dsh-session-revision` — requires `agents` · `sessions` · `llm` ([`packages/session/session-revision/src/index.ts`](../packages/session/session-revision/src/index.ts))
