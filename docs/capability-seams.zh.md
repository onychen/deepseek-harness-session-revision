...
| `ctx.sessionRevision` | `core` | [`session-revision`](../packages/session/session-revision) | - | [`client-ui-session-revision`](../packages/client/ui-session-revision) | - | 校验空闲同 Session 的编辑与重新生成，提交 append-only retract 记录，并排入替换 prompt。 |
