...
describe('session revision UI', () => {
  ...
  it('submits user withdrawal against the displayed raw tail', async () => {
    const revise = vi.fn().mockResolvedValue({ kind: 'committed', retractSeq: 10, persistence: 'persisted' })
    const props = {
      seq: 4,
      text: 'historical prompt',
      t: (key: string) => key,
      useInput: (select: (value: unknown) => unknown) => select({ draft: '', imageIds: [] }),
      useSession: (select: (value: unknown) => unknown) => select({
        running: false, queue: [], lastSeq: 9, subagent: null, removed: false,
      }),
    ...
    expect(revise).toHaveBeenCalledWith('withdraw', expect.objectContaining({ targetSeq: 4, expectedLastSeq: 9 }))
  })
