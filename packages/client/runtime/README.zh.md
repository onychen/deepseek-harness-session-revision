...
`session/retract` 之后，历史响应可以包含稀疏的当前分支窗口；原始 `seq` 保持不变，响应会标记允许存在的间隙。Session 将原始传输窗口与有效分支窗口作为两个不同概念维护。历史 replace、prepend、重连和实时 retract 都先经同一个分支投影重建，再交给 Conversation 组装，因此被移除节点会消失，而后续事件仍可正常追加。
