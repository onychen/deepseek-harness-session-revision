...
- `surfaceOp?: SurfaceOp`：事件进入 surface 的方式。`session/retract` 携带 `{ op: 'delete', from }`，把当前历史恢复为原始目标事件之前的有效前缀，同时保留原始后缀用于审计。其他非 surface 事件不含该字段。
...
- **`SESSION_FORMAT_VERSION` 为 `1`**：v1 增加 delete surface 操作；由于 v0 是严格子集，持久化边界只升级 v0 header，无需重写事件。不认识的事件类型仍会被拒绝，除非信封带 `ignorable` 标记；机制见 [session-log 版本机制 Agent Note](../../../.agents/notes/implemented/architecture/2026-08-10-session-log-version-mechanism.md)。
