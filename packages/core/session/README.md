...
- `surfaceOp?: SurfaceOp` — how this event entered the surface. `session/retract` carries `{ op: 'delete', from }`, restoring current history to the effective prefix before the raw target event while retaining the raw suffix for audit. Absent for other non-surface events.
...
- **`SESSION_FORMAT_VERSION` is `1`** — v1 adds the delete surface operation; the persistence boundary upgrades v0 headers without rewriting events because v0 is a strict subset. Unknown event types are rejected unless marked `ignorable`; the mechanism is the [session-log-version-mechanism note](../../../.agents/notes/implemented/architecture/2026-08-10-session-log-version-mechanism.md).
